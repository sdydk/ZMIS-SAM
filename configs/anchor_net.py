...
default_hooks = dict(
    ...
    checkpoint=dict(type='CheckpointHook', interval=20, max_keep_ckpts=3, save_best=['coco/bbox_mAP', 'coco/segm_mAP'],
...
# sam huge model
sam_pretrain_name = "/root/autodl-tmp/ZMIS-SAM/pretrain/sam-vit-huge"
sam_pretrain_ckpt_path = "/root/autodl-tmp/ZMIS-SAM/pretrain/sam-vit-huge/pytorch_model.bin"
...
model = dict(
    ...
    adapter=dict(
        type='ZViTAdapters', adapter_layer=range(8, 33, 2), embed_dim=1280
    ),
    neck=dict(
        type='ZMISFPN',
        feature_aggregator=dict(
            type='ZMISFeatureAggregator',
    ...
    roi_head=dict(
        type='ZMISPrompterAnchorRoIPromptHead',
        with_extra_pe=True,
        ...
        mask_roi_extractor=dict(
            ...
            featmap_strides=[4, 8, 16, 32]),
        mask_head=dict(
            type='ZMISPrompterAnchorMaskHead',
            mask_decoder=dict(
                type='ZMISSamMaskDecoder',
                hf_pretrain_name=sam_pretrain_name,
                init_cfg=dict(type='Pretrained', checkpoint=sam_pretrain_ckpt_path)),
            in_channels=256,
            roi_feat_size=14,
