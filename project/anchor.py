...
from .common import NFAM
from .wavelet import tDWT
import matplotlib.pyplot as plt
from .norm import LayerNorm2d
from .multi_fea_enh import WM2FE

@MODELS.register_module()
class ZMISAnchor(MaskRCNN):
    def __init__(
            self,
        ...
        self.wt = tDWT(n_level=6, wave='haar', alpha=1.5, beta=1.2) 
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        x_gray = rgb_to_grayscale(batch_inputs).to(batch_inputs.device)
        score = self.wt(x_gray)
        ...
        x = self.neck(vision_hidden_states, dfb_list, low_list)
...
class ZMISFPN(BaseModule):
    ...
    def forward(self, inputs, dfb_list=None, low_list=None):
        if hasattr(self, 'feature_aggregator'):
            x = self.feature_aggregator(inputs,)
        else:
            x = inputs
        if hasattr(self, 'feature_spliter'):
            x = self.feature_spliter(x, dfb_list, low_list)
        else:
            x = (x,)
        return x

@MODELS.register_module()
class ZMISSimpleFPNHead(BaseModule):
    def __init__(self,
        ...
        for i in range(self.num_outs):
            ...
            self.wm2fes.append(wm2fe)
            

    def forward(self, input: Tensor, dfb_list=None, low_list=None) -> tuple:
        """Forward function.
        ...
        """
        # original_features = [einops.rearrange(x, 'b h w c -> b c h w') for x in original_features]
        # last_feature = original_features[32]
        # last_feature = self.channel_reduction(last_feature)
        # dfb_list = dfb_list[:4]
        # build FPN
        inputs = []
        ...
        midds = [self.fpn_convs[i](laterals[i]) for i in range(self.num_ins)]

        # part 2: add extra levels
        if self.num_outs > len(midds):
            for i in range(self.num_outs - self.num_ins):
                midds.append(F.max_pool2d(midds[-1], 1, stride=2))
        outs = []
        for i in range(self.num_outs):
            outs.append(self.wm2fes[i](midds[i], dfb_list[i]))

        return tuple(outs)


@MODELS.register_module()
class ZMISFeatureAggregator(BaseModule):
    def __init__(
            self,
            in_channels,
            select_layers,
        ...
        self.downconvs_1 = nn.ModuleList()
        self.hidden_convs = nn.ModuleList()
        self.nfams = nn.ModuleList()
        ...
        for i_layer in self.select_layers:
            ...
            self.downconvs_1.append(
                nn.Sequential(
                    nn.Conv2d(self.in_channels[i_layer] * 2, hidden_channels* 2, 1),
    ...
    def forward(self, inputs):
        ...
        for idx, i_layer in enumerate(self.select_layers):
            features1.append(
                self.downconvs[idx](inputs[i_layer-1])
            )
            features2.append(
                self.downconvs[idx](inputs[i_layer])
            )
            features3.append(
                self.downconvs_1[idx](torch.cat([inputs[i_layer-1], inputs[i_layer]], dim=1))
            )

        x = None
        for hidden_state1, hidden_state2, hidden_state3, hidden_conv, nfamnet in zip(features1, features2, features3, self.hidden_convs, self.nfams):
            hidden_state = nfamnet(hidden_state1, hidden_state2, hidden_state3)
            if x is not None:
...
@MODELS.register_module()
class ZMISPrompterAnchorRoIPromptHead(StandardRoIHead):
    def __init__(
            self,
            with_extra_pe=False,
            *args,
...
@MODELS.register_module()
class ZMISPrompterAnchorMaskHead(FCNMaskHead, BaseModule):
    def __init__(
            self,
            mask_decoder,
            in_channels,
